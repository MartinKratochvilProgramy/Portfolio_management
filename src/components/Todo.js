...
export default function Todo({ todo, toggleComplete, deleteTodo }) {
  ...
  return (
    <div className={todo.done ? "border-green-600 border-solid border-[1px] rounded my-6" : "border-blue-600 border-solid border-[1px] rounded my-6"}>
      <div className="flex flex-row items-center px-7 py-3 my-0 text-black font-medium text-sm leading-snug uppercase whitespace-nowrap hover:shadow-xl focus:shadow-lg focus:outline-none focus:ring-0 active:bg-blue-800 active:shadow-lg transition duration-150 ease-in-out">
        {todo.done? (
          <button onClick={handleToggleClick} className="relative group rounded-full p-1 transition duration-150 hover:bg-slate-100 ease-in-out">
            ...
            <div className="absolute z-10 py-1 px-2 rounded-md bottom-[-35px] bg-slate-100 hidden group-hover:block transition delay-300">
              Toggle not complete
            </div>
          </button>
        ) : (
